...
def get_max_iter(spin, omega, gamma_g, gamma_d, coupling_strength, delta_min, delta_max, num_points):
    ...
    sparse_liouvillian = get_sparse_liouvillian(*construct_spin_network_gen_coupling(spin, omega_arr,
                                                coupling_matrix, [gamma_d] * 3, gamma_g_arr))
    ...
    dev_exp_liouvillian = (cp_eye(opr_len, dtype=cp.complex128, format='csr') + cp_csr_matrix(
        (cp.array(sparse_liouvillian.data),
         ...
         cp.array(sparse_liouvillian.indptr)), shape=(opr_len, opr_len), dtype=cp.complex128) * cp.float64(final_t_to_n_ratio))
    try:
...
#                 cp.array(sparse_liouvillian.indptr)), shape=(opr_len, opr_len), dtype=cp.complex128)
            ...
            final_time_guess = 5
            final_t_to_n_ratio = 5e-4  # Smaller values of this will give more accurate results, but take longer to compute
            num_iterations = int(np.ceil(np.log2(final_time_guess / final_t_to_n_ratio)))
            ...
            dev_exp_liouvillian = (cp_eye(opr_len, dtype=cp.complex128, format='csr') + cp_csr_matrix((cp.array(sparse_liouvillian.data), cp.array(sparse_liouvillian.indices), cp.array(sparse_liouvillian.indptr)), shape=(opr_len, opr_len), dtype=cp.complex128) * cp.float64(final_t_to_n_ratio))
            counter = 0
            ...
            while not np.isclose(np.linalg.norm(sparse_liouvillian @ dev_result.get()), 0, atol=tolerance):
                ...
                # print("made it here {}".format(second_loop_iterations))
                # sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    max_iter = 0 #get_max_iter(spin, omega, gamma_g, gamma_d, coupling_strength, delta_min, delta_max, num_points)
